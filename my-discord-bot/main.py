    ...
    async def _load_extensions(self) -> None: # Load our 'modules' we make in /cogs.
        ...
        for filename in os.listdir(self.ext_dir):
            if filename.endswith(".py") and not filename.startswith("_"):
                try:
                    await self.load_extension(f"cogs.{filename[:-3]}") # They need to be, for example, `cogs.command_cog`.
...
def main() -> None:
    bot = WSUBot(prefix="$", ext_dir="my-discord-bot/cogs") # ext_dir begins at root ie Discord-bot/.
    bot.run()

if __name__ == "__main__": # If supposed to be running main, run our defined main function. 
